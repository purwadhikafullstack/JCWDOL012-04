...
export default function TransactionHistory() {
    ...
    const [productCategory, setProductCategory] = useState<ProductCategoriesModel[]>([]);
    ...
    useEffect(() => {
        ...
        transactionApi.getAllProductCategory().then((response) => {
    ...
    if (searchByOrderIdTerm!=='' || searchTerm !== '' || category !== '' || startDate || endDate || orderStatusTerm) {
        filteredTransactions = transactions.filter(transaction => {
            const orderIdMatch = transaction.transactionUid.toLowerCase().includes(searchByOrderIdTerm.toLowerCase());
            const productMatchName = transaction.products?.some(product =>
                product.product?.name.toLowerCase().includes(searchTerm.toLowerCase())
            );
            const productMatchCategory = transaction.products?.some(product =>
                product.product?.productCategory.name.includes(category)
            );
            ...
            return orderIdMatch && productMatchCategory && productMatchName && dateMatch && orderStatusMatch;
        });
    }
    ...
    return (
        <div className="px-[24px] h-fit 2xl:mx-64">
