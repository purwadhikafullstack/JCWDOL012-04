...
export default function CartAdd({ productId }: { productId: number }) {
    ...
    useEffect(() => {
        cartApi.preAddToCart(productId).then((response) => {
            setLoading(true);
    ...
    async function handleAddToCart() {
        if (stock < 0) {
            ...
            await cartApi.addToCart(productId, quantity).then((response) => {
                setLoading(true);
                setStatus(response.status);
                if (response.status === 200) {
                ...
                toast.error('Failed to add to cart. Please contact our customer service');
                setStatus(error.status);
                console.log(error);
            })
        }
    ...
    let inputLimit = isExceedQuantity ? 'border-10 border-red-600' : basicInputBorder;
    ...
    return (
        <div className='m-5 p-5 border-2 border-[var(--primaryColor)] rounded-xl lg:max-w-[350px] lg:max-h-[250px] lg:min-h-fit'>
            ...
            <div className='ml-5'>
                ...
                {(isExceedQuantity&&quantity>0) && <p className='text-red-500 '>Reached Stock Limit!</p>}
                {(isExceedQuantity&&quantity<0) && <p className='text-red-500 '>Please Enter Valid Amount!</p>}
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <div className='flex text-xl'>
                        ...
                        <p className='font-bold text-right '>{(quantity>0 && !isExceedQuantity)?idr(product?.price ? product?.price * quantity : 0):'-'}</p>
                    </div>
                </div>
                <div className='flex justify-end mt-5'>
                    <button className={`bg-[var(--primaryColor)] hover:bg-[var(--primaryColor-dark)] text-white px-5 py-2 rounded-lg ${isExceedQuantity ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={isExceedQuantity} onClick={handleAddToCart}>Add to Cart</button>
                </div>
            </div>
        </div>
    )
